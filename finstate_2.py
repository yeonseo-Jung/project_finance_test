...
# finstate.py
path = "C:\\Users\\wjddu\Desktop\\projects\\finstate_all_tables"
account_nm = ["equity", "profit"]
account_id = [["ifrs_EquityAttributableToOwnersOfParent", "ifrs-full_EquityAttributableToOwnersOfParent"], ["ifrs_ProfitLossAttributableToOwnersOfParent", "ifrs-full_ProfitLossAttributableToOwnersOfParent"]]
try:
    f = open(f'{path}\\accounts.txt', 'x')
    i = 0
    for ac_nm in account_nm:
        ac_id = account_id[i]
        make_accounts(ac_nm, ac_id, path)
        i += 1
